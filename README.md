...
화면과 촬영 흐름만 확인할 때는 로그인 게이트를 건너뛸 수 있다.

```bash
flutter run -d chrome --dart-define=ALLOW_GUEST=true --dart-define=VWORLD_KEY=...
```

이 플래그는 **빌드할 때만** 켜진다. 배포 워크플로는 넘기지 않으므로 배포본은
언제나 로그인 화면에서 시작한다.

게스트 상태에서 조사 작성·촬영·약도는 모두 되지만 드라이브로는 아무것도 나가지
않는다. 사진은 기기에 쌓이고, 목록 화면 오른쪽 위 **로그인**을 누르면 그때
대기 중이던 것이 한꺼번에 올라간다. 이 상태라는 것을 놓치지 않도록 목록과 촬영
화면에 안내 줄이 계속 붙는다.
